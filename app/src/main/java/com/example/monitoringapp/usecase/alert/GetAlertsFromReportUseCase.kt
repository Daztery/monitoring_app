...
class GetAlertsFromReportUseCase @Inject constructor(private val alertRepository: AlertRepository) {
    suspend operator fun invoke(reportId: Int): OperationResult<CollectionResponse<Alert>> {
        return alertRepository.getAlertsFromReport(reportId)
    }
}