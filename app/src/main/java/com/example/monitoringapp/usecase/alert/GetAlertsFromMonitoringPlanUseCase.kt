...
class GetAlertsFromMonitoringPlanUseCase @Inject constructor(private val alertRepository: AlertRepository) {
    suspend operator fun invoke(planId: Int): OperationResult<CollectionResponse<Alert>> {
        return alertRepository.getAlertsFromMonitoringPlan(planId)
    }
}